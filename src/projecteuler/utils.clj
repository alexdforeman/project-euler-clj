(ns projecteuler.utils
  ...
  (:require [clojure.core.matrix :as matrix :only [diagonals transpose]])
  )
...
(def problem-011-matrix [[  8  2 22 97 38 15  0 40  0 75  4  5  7 78 52 12 50 77 91  8]
...
(defn gen-matrix-diagonals [m]
  (loop [size (count (first problem-011-matrix))
         index (- (inc size))
         diagonals ()
         ]
    (cond (>= index size) (filter (comp not empty?) (into [] diagonals))
          :else
          (recur size (inc index) (cons (matrix/diagonal m index) diagonals))
          )))

(defn product-group
  ([v] (product-group v 4))
  ([v n]
     "Find the max product of sequential number with group size n from v"
     (cond (empty? v) []
           :else (cons (reduce * (take n v))
                       (product-group (rest v) n ))
           ))
  )
(defn max-product-group
  ([v] (max-product-group v 4))
  ([v n]
     (apply max (product-group v n)))
  )

(defn gen-all-matrixs [m]
  (lazy-cat m
            (matrix/transpose m)
